...
# Les Kunst og håndverk-filen
with open('/Users/kennethbareksten/Downloads/Kunst og håndverk KHV01-02.txt', 'r', encoding='cp1252') as f:
    content = f.read()

# Kjerneelementer for Kunst og håndverk (fra curriculumData.ts)
CORE_ELEMENTS = [
    "Utforske",
    "Produsere",
    "Reflektere"
...
def parse_goals():
    ...
    sections = re.split(r'Kompetansem.l etter (\d+)\.?\s*trinn', content)
    ...
    for i in range(1, len(sections), 2):
        ...
        goal_objects = []
        base_code = {"2": 1, "4": 10, "7": 20, "10": 31}[level]
...
# Parse målene
khv_data = parse_goals()

# Les eksisterende JSON
with open('data/competenceGoals.json', 'r', encoding='utf-8') as f:
    all_data = json.load(f)

# Erstatt Kunst og håndverk
all_data['KHV'] = khv_data

# Skriv tilbake
with open('data/competenceGoals.json', 'w', encoding='utf-8') as f:
    json.dump(all_data, f, ensure_ascii=False, indent=2)

print("\n✓ Kunst og håndverk oppdatert!")
print(f"Totalt: {sum(len(goals) for goals in khv_data['levels'].values())} mål")
